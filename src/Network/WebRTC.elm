...
module Network.WebRTC exposing
    ( Model
    , init
    , Action(..)
    , initialize
...
initRoom peers users me model = undefined
  -- { model |
...
initialize selfId iceServers =
  initialize_ selfId iceServers `onError` (\e -> fail <| Error e)
...
initialize_ : PeerId -> List Json.Encode.Value -> Task String ()
initialize_ = Native.WebRTC.initialize
